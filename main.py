...
# Ensure frontend directory exists so FastAPI doesn't crash on startup
...
KNOWN_ABBR = {
    ...
    "University of California, Irvine":"UCI","University of California, Davis":"UCD",
    "University of California,  Riverside":"UCR","University of Southern California":"USC",
    "University of Michigan, Ann Arbor":"UMich","University of Maryland, College Park":"UMD",
    "University of Illinois Urbana-Champaign":"UIUC","University of Illinois at Chicago":"UIC",
    "The University of Texas at Austin":"UTA","The University of Texas at Dallas":"UTD",
    "The University of Texas at Arlington":"UTA","University of Washington":"UW",
    "University of Wisconsin-Madison":"UW-M","University of Minnesota, Twin Cities":"UMN",
    "University of Florida":"UFL","University of North Carolina at Charlotte":"UNCC",
    "University of Massachusetts Amherst":"UMass","University of Colorado Boulder":"CUB",
    "Carnegie Mellon University":"CMU","Georgia Institute of Technology":"GT",
    "Columbia University":"CU","New York University":"NYU",
    "Northeastern University, Boston":"NEU","Northeastern University, Silicon Valley":"NEU-SV",
    "Boston University":"BU","Purdue University West Lafayette":"Purdue",
    "Pennsylvania State University":"PSU","Texas A&M University, College Station":"TAMU",
    "Arizona State University":"ASU","University at Buffalo SUNY":"UB",
    "Stony Brook University":"SBU","Stevens Institute of Technology":"SIT",
    "Illinois Institute of Technology":"IIT","North Carolina State University, Raleigh":"NCSU",
    "Virginia Tech":"VT","Duke University":"Duke","Johns Hopkins University":"JHU",
    "Northwestern University":"NU","Cornell University":"Cornell",
    "Stanford University":"Stanford","Harvard University":"Harvard",
    "The University of Chicago":"UChicago","University of Chicago":"UChicago",
    "Vanderbilt University":"Vandy","Indiana University Bloomington":"IU",
    "George Mason University":"GMU","The George Washington University":"GWU",
    "Rutgers University, New Brunswick":"RU","San Jose State University":"SJSU",
    "New Jersey Institute of Technology":"NJIT","Rochester Institute of Technology":"RIT",
    "University of Arizona":"UA","University of Maryland, Baltimore County":"UMBC",
    "California State University, Long Beach":"CSULB","University of Pittsburgh":"Pitt",
    "Pace University":"Pace","University of Cincinnati":"UC",
    "University College London":"UCL","Imperial College London":"ICL",
    "The University of Edinburgh":"UoE","University of Edinburgh":"UoE",
    "King's College London":"KCL","University of Manchester":"UoM",
    "University of Birmingham":"UoB","University of Bristol":"UoB",
    "University of Warwick":"UoW","University of Glasgow":"UoG",
    "University of Liverpool":"UoL","Queen Mary University of London":"QMUL",
    "University of Nottingham":"UoN","University of Leeds":"Leeds",
    "Technical University of Munich":"TUM","RWTH Aachen University":"RWTH",
    "Dresden University of Technology":"TU Dresden",
    "Delft University of Technology":"TU Delft",
    "University of Toronto":"UoT",
    "The University of British Columbia, Vancouver":"UBC","McGill University":"McGill",
}
...
def run_pipeline(base, we_lo, we_hi, eng_test, eng_lo, eng_hi, limit=10):
    ...
    pipeline += [
        {"$group": {
            "_id":"$university","admit_count":{"$sum":1},
            "rank":{"$first":"$rank"},"rank_label":{"$first":"$rank_label"},
    ...
    for offset in [1,2]:
        ...
        pipeline = [
            ...
            {"$group":{
                "_id":"$university","admit_count":{"$sum":1},
                "rank":{"$first":"$rank"},"rank_label":{"$first":"$rank_label"},
                "courses":{"$addToSet":"$course"},"min_eng":{"$min":"$profile.eng_score"},
            }},
        ...
        for r in col.aggregate(pipeline):
            ...
            label = (f"IELTS {actual}+ required · you have {eng_score}"
                     if t=="IELTS" else f"TOEFL {int(actual)}+ required · you have {int(eng_score)}")
            out.append(NearMiss(
                university=r["_id"],course=sorted(r["courses"])[0] if r["courses"] else "",
                rank_label=r.get("rank_label"),gap_field="english_score",gap_label=label,
                user_value=eng_score,required_value=actual,admit_count=r["admit_count"],
            ))
            seen.add(r["_id"])
    return out[:2]
...
def predict(req: PredictRequest):
    ...
    if not rows:
        # Added debug logs to help you verify what data is missing from the database
        print(f"\n[DEBUG] Database returned empty for query:")
        print(f"[DEBUG] Country: '{country_db}', Course: '{course_db}', Degree: '{degree_db}'\n")
        raise HTTPException(422, f"No data found for '{req.parent_course}' in {req.country}. Please check if your MongoDB has matching data.")

    seen=set(); cards=[]
    for r in rows:
        pct = math.floor(r["schol_count"]/r["admit_count"]*100) if r["admit_count"] else 0
        cards.append(OfferCard(
            university=r["_id"],course=sorted(r["courses"])[0] if r["courses"] else course_db,
            rank_label=r.get("rank_label"),rank=r.get("rank"),
            admit_count=r["admit_count"],scholarship_pct=pct,
            profile_url=r.get("profile_url"),abbr=make_abbr(r["_id"]),
        ))
        seen.add(r["_id"])
    ...
    return PredictResponse(
        matched_profiles=total, results=cards, near_miss=nm,
        fallback_used=fallback, context_label=" · ".join(parts),
    )
